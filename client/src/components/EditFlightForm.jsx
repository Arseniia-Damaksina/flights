...
const EditFlightForm = () => {
    ...
    useEffect(() => {
        if (flightToUpdate) {
            setFlight_number(flightToUpdate.flight_number || '');
            setAirline(flightToUpdate.airline || '');
            setDeparture_city(flightToUpdate.departure_city || '');
            setDeparture_airport(flightToUpdate.departure_airport || '');
            setDeparture_time(flightToUpdate.departure_time || '');
            setArrival_city(flightToUpdate.arrival_city || '');
            setArrival_airport(flightToUpdate.arrival_airport || '');
            setArrival_time(flightToUpdate.arrival_time || '');
            setDuration(flightToUpdate.duration || '');
            setPrice(flightToUpdate.price || 0);
        }
    }, [flightToUpdate]);
    ...
    const handleSubmit = (e) => {
        ...
        const editFlight = async (id) => {
            try {
                ...
                console.log(res);
                if (res.status !== 200) {
                    throw new Error('Error while editing a flight');
                } else {
                    if (res.data.success) {
